...
(defn relation
  ...
  (if-not (contains? attrs :project_id)
    (throw (AssertionError. "Missing required key :project_id")))
  (if-not (contains? attrs :parent_entity_id)
    (throw (AssertionError. "Missing required key :parent_entity_id")))
  (if-not (contains? attrs :parent_entity_type)
    (throw (AssertionError. "Missing required key :parent_entity_type")))
  (if-not (contains? attrs :child_entity_id)
    (throw (AssertionError. "Missing required key :child_entity_id")))
  (if-not (contains? attrs :child_entity_type)
    (throw (AssertionError. "Missing required key :child_entity_type")))
  (let [record (merge {:_id (str (util/make-uuid))
                       :rel "Parent"
