...
(facts "About links"
    ...
    (let [doc1 {:attributes {:label ..label1..}}
      ...
      (against-background [(couch/get-view ..auth.. ..db.. k/LINKS-VIEW {:startkey      [..id.. ..rel..]
        ...
        (fact "gets entity rel targets"
          (links/get-link-targets ..auth.. ..db.. ..id.. ..rel.. ..rt..) => [doc1 doc2 doc3])

        (fact "gets named entity rel targets"
          (links/get-link-targets ..auth.. ..db.. ..id.. ..rel.. ..rt.. :name ..name..) => [doc1])

        (fact "filters by label"
          (links/get-link-targets ..auth.. ..db.. ..id.. ..rel.. ..rt.. :label ..label1..) => [doc1]))))
  ...
  (facts "`add-link`"
    (let [doc {:_id   (str (UUID/randomUUID))
      ...
      (against-background [(core/get-entities ..auth.. ..db.. [target-id] ..rt..) => [{:_id   target-id
                                                                                :type  "not-a-root"
        ...
        (fact "creates link document"
          (:links (links/add-links ..auth.. ..db.. [doc] ..rel.. [target-id] ..rt.. :inverse-rel ..inverse..)) => (contains {:_id         (format "%s--%s-->%s" (:_id doc) ..rel.. target-id)
                                                                                                                      :user_id     ..id..
                                                                                                                      :type        "Relation"
                                                                                                                      :source_id   (:_id doc)
                                                                                                                      :target_id   target-id
        ...
        (fact "creates link document without inverse"
          (:links (links/add-links ..auth.. ..db.. [doc] ..rel.. [target-id] ..rt..)) => (contains {:_id       (format "%s--%s-->%s" (:_id doc) ..rel.. target-id)
                                                                                             :user_id   ..id..
                                                                                             `:type     "Relation"
                                                                                             :source_id (:_id doc)
                                                                                             :target_id target-id
        ...
        (fact "creates named link document"
          (:links (links/add-links ..auth.. ..db.. [doc] ..rel.. [target-id] ..rt.. :inverse-rel ..inverse.. :name ..name..)) => (contains {:_id         (format "%s--%s>%s-->%s" (:_id doc) ..rel.. ..name.. target-id)
                                                                                                                                     :user_id     ..id..
                                                                                                                                     :type        "Relation"
                                                                                                                                     :source_id   (:_id doc)
                                                                                                                                     :target_id   target-id
        ...
        (fact "updates source _collaboration_roots by aggregating multiple target collaboration roots"
          (let [rel "some-rel"
            ...
            (:updates (links/add-links ..auth.. ..db.. [source] rel [target-id target-id2] ..rt..)) => (contains updated-source)
            (provided
              (core/get-entities ..auth.. ..db.. [target-id target-id2] ..rt..) => [target1 target2])))
        ...
        (fact "updates target _collaboration_roots for source:=Project"
          (let [rel "some-rel"
            ...
            (:updates (links/add-links ..auth.. ..db.. [source] rel [target-id] ..rt..)) => (contains expected)
            (provided
              (core/get-entities ..auth.. ..db.. [target-id] ..rt..) => [target])))
      ...
      (against-background []
        ...
        (fact "removes link"
          (let [source-id (:_id doc)
            ...
            (links/delete-links ..auth.. ..db.. ..rt.. doc ..rel.. ..target..) => ..deleted..
            (provided
              (core/delete-values ..auth.. ..db.. [link-id] ..rt..) => ..deleted..
              (auth/can? ..auth.. ::auth/update doc) => true)))

        (fact "fails if not can? :update source"
          (links/delete-links ..auth.. ..db.. ..rt.. ..doc.. ..rel.. ..target..) => (throws Exception)
          (provided
